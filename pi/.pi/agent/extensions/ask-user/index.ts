...
export default function askUser(pi: ExtensionAPI) {
  let removeRelay: (() => void) | undefined;
  let isRelayedChild = false;
  let awaitingClarification = false;

  pi.on("session_start", (_event, ctx) => {
    isRelayedChild = ctx.mode === "print";
    if (ctx.mode !== "tui") return;
    removeRelay?.();
    removeRelay = installAskUserRelay(ctx.ui, (markdown) => {
      pi.sendMessage(
        {
          customType: "ask-user-clarification",
          content: markdown,
          display: true,
        },
  ...
  pi.on("message_end", (event) => {
    if (!isRelayedChild || !awaitingClarification) return;
    if (event.message.role !== "assistant") return;

    const clarification = assistantText(event.message);
    if (!clarification) return;
    awaitingClarification = false;
    surfaceAskUserClarification(clarification);
  });

  pi.on("session_shutdown", () => {
    removeRelay?.();
    removeRelay = undefined;
    isRelayedChild = false;
    awaitingClarification = false;
  });
  ...
  pi.registerMessageRenderer(
    ...
    (message, _options, theme) => {
      ...
      const heading = new Text(
        theme.fg("accent", theme.bold("clarification")),
        0,
        0,
      );
      const body = new Markdown(text, 0, 0, getMarkdownTheme());
      return {
        render: (width: number) => [
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, signal, _onUpdate, ctx) {
      ...
      const questionUi =
        ctx.mode === "tui" ? ctx.ui : getAskUserRelay();
      if (!questionUi) {
      ...
      const showQuestion = (uiSignal: AbortSignal) =>
        questionUi.custom<SelectionResult>((tui, theme, _kb, done) => {
          let optionIndex = 0;
          let editMode = false;
      ...
      if (result.wasCustom) {
        // Only relayed child sessions need to surface the next text response.
        // The primary session already renders its own assistant message.
        if (isRelayedChild) awaitingClarification = true;
        return reply(
          buildAskUserResultMessage({
            kind: "custom",
