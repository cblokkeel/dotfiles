...
import { generateSessionTitle, summarizeRun } from "./src/summarizer.ts";
import {
  buildFallbackRecap,
  createRunBoundary,
  getRunEntries,
...
function hasSessionNameEntry(ctx: ExtensionContext) {
...
export default function (pi: ExtensionAPI) {
  ...
  let sessionNamingReserved = false;
  ...
  const updateStatus = () => {
    statusContext?.ui.setStatus(
      ...
      activeSummaries.size > 0
        ? statusContext.ui.theme.fg("muted", "✦ updating session…")
        : undefined,
    );
  };

  const startSessionNaming = (prompt: string, ctx: ExtensionContext) => {
    if (sessionNamingReserved || hasSessionNameEntry(ctx)) return;

    sessionNamingReserved = true;
    const controller = new AbortController();
    const config = loadSummaryConfig();
    const task = generateSessionTitle({
  ...
  pi.on("before_agent_start", (event, ctx) => {
    ...
    startSessionNaming(event.prompt, ctx);
  });
  ...
  pi.on("agent_settled", (_event, ctx) => {
    ...
    const shouldNameSession =
      !sessionNamingReserved && !hasSessionNameEntry(ctx);
    if (shouldNameSession) sessionNamingReserved = true;
    const controller = new AbortController();
    statusContext = ctx;
    const task = (async () => {
      ...
      if (shouldNameSession && !recap.fallback && !hasSessionNameEntry(ctx)) {
        pi.setSessionName(recap.title);
      }
    })().finally(() => {
      if (shouldNameSession) sessionNamingReserved = false;
      activeSummaries.delete(controller);
      updateStatus();
    });
