...
const FAKE_CARET_PATTERN = /\x1b\[7m(.*?)\x1b\[(?:0|27)m/;

export function hideFakeCaret(lines: string[]) {
  let hidden = false;

  return lines.map((line) => {
    if (hidden || !FAKE_CARET_PATTERN.test(line)) return line;
    hidden = true;
    return line.replace(FAKE_CARET_PATTERN, "$1");
  });
}
...
class FocusAwareEditor extends CustomEditor {
  ...
  constructor(
    ...
    this.isTerminalFocused = isTerminalFocused;
  }

  override render(width: number): string[] {
    const lines = super.render(width);
    return this.isTerminalFocused() ? lines : hideFakeCaret(lines);
  }
}

function setFocusReporting(enabled: boolean) {
